...
while running:
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            for buoy in buoys:
                if math.hypot(mouse_pos[0] - buoy.x, mouse_pos[1] - buoy.y) < buoy.radius:
                    ...
                    # Создание элементов окна параметров поплавка
                    UILabel(
            ...
            for i, wave in enumerate(waves):
                ...
                if abs(mouse_pos[1] - y_offset) < 30:
        ...
        if event.type == pygame_gui.UI_BUTTON_PRESSED:
            if event.ui_element == add_wave_button:
                ...
                if selected_wave_index < len(waves):
                    removed_wave = waves.pop(selected_wave_index)
                    # Обновляем ID оставшихся волн
                    for i, wave in enumerate(waves):
                        wave.id = str(i + 1)
                    # Обновляем поплавки
                    new_buoys = []
    ...
    # Отрисовка волн
    for i, wave in enumerate(waves):
        # Пересчет y_offset на основе текущего индекса
        y_offset = WAVE_SPACING * (i + 1)
        points = []
